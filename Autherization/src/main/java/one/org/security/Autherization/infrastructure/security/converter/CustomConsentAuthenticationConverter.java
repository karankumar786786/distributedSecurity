...
public class CustomConsentAuthenticationConverter implements AuthenticationConverter {
    ...
    public Authentication convert(HttpServletRequest request) {
        ...
        log.debug("CustomConsentConverter - Attempting conversion for {}", request.getRequestURI());

        String clientId = request.getParameter(OAuth2ParameterNames.CLIENT_ID);
        String state = request.getParameter(OAuth2ParameterNames.STATE);

        log.debug("CustomConsentConverter - client_id: {}, state: {}", clientId, state);

        if (!StringUtils.hasText(clientId) || !StringUtils.hasText(state)) {
            log.debug("CustomConsentConverter - Missing client_id or state. Returning null.");
            // We should let the delegate fail or handle it, but logging is key here.
        }

        Authentication principal = SecurityContextHolder.getContext().getAuthentication();
        log.debug("CustomConsentConverter - Principal: {}", principal);
        if (principal == null || !principal.isAuthenticated()) {
            log.debug("CustomConsentConverter - Principal is null or not authenticated!");
        }

        Authentication result = delegate.convert(request);
        if (result == null) {
            log.debug("CustomConsentConverter - Delegate returned NULL. Conversion failed.");
        } else {
            log.debug("CustomConsentConverter - Conversion SUCCESS: {}", result);
        }
        return result;
    }
}
